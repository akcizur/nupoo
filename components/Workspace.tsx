...
function Breadcrumbs({ pages, current, onSelect }: { pages: Page[]; current?: Page; onSelect: (id: string) => void }) {
  const chain = useMemo(() => {
    const result: Page[] = []
    let page = current?.parentId ? pages.find((item) => item.id === current.parentId && !item.trashedAt) : undefined
  ...
  return (
    <div className="flex min-w-0 items-center gap-1 overflow-hidden text-[11px] text-muted-foreground">
      <span className="shrink-0 font-medium text-foreground/70">Nupoo</span>
      {chain.map((page) => (
        <span key={page.id} className="flex min-w-0 items-center gap-1">
          <span aria-hidden className="text-muted-foreground/30">/</span>
          <button onClick={() => onSelect(page.id)} className="max-w-[140px] truncate rounded-[5px] px-1 py-1 transition-colors hover:bg-accent hover:text-accent-foreground">
    ...
    <div className={`min-h-screen bg-background text-foreground ${focusMode ? 'focus-mode' : ''}`}>
      ...
      <div className="flex min-h-screen">
        ...
        <AnimatePresence>{!focusMode && mobileSidebar && <>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-40 bg-black/40 md:hidden" onMouseDown={() => setMobileSidebar(false)} />
        ...
        <main className="min-w-0 flex-1">
          {!focusMode && (
            <header className="sticky top-0 z-30 flex h-12 items-center gap-1 border-b border-border bg-background/92 px-2 sm:px-3">
              ...
              <div className="hidden items-center gap-1 px-1 text-[10px] font-medium text-muted-foreground/70 lg:flex" aria-live="polite">
          ...
          <motion.section key={activePageId} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: .12, ease: 'easeOut' }} className="w-full px-0 pb-0 pt-0">
